...
class Messages(core.Core):
    ...
    def __init__(self):
        ...
        self._args_empty = self._('[{}{}{}] Словарь аргументов командной строки пуст ...')
        self._check_args_valid = self._('[{}] Проверка аргументов командной строки на валидность ...')
        self._create_dirs = self._('[{}] Создание каталогов для служебных файлов ...')
        self._download = self._('[{}] Загрузка "{}" ...')
        self._extract = '    ' + self._('Извлечение данных из "{}" ...')
        self._index_error = '    ' + self._('{}Категория "{}" не найдена, пропуск ... {}')
        self._missing_file = '    ' + self._('Отсутствует файл "{}" ...')
        self._automatic_download = '        ' + self._('Автоматическая загрузка ...') + ' '
        self._input = '        ' + self._('Скачать отсутствующий файл? [Y/n]') + ' '
        self._input_error = '{}' + self._input + '{}'
        self._cancel_download = self._('[{}{}{}] Отмена загрузки ...')
        self._url_error = '        ' + self._('[{}{}{}] Не удалось скачать файл (ошибка {}) ...')
        self._already_downloaded = '        ' + self._('Все изображения были загружены ранее ...')
        self._all_images_in_class = '    ' + self._('Всего "{}" изображений {} ...')
        self._images_not_found = '    ' + self._('{}Изображений "{}" не найдено, пропуск ...{}')
        self._limit = ' ' + self._('из них будет загружено {} ...')
        self._labels = '    ' + self._('Формирование меток ...') + ' '
        self._photo_not_read = '        ' + self._('{}Изображение "{}" повреждено ...{}')
        self._headers = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_5) AppleWebKit/537.36 (KHTML, like Gecko) ' \
                        'Chrome/83.0.4103.61 Safari/537.36'
        self._in_developing = self._('[{}{}{}] {} в разработке ...')
...
class OIDv6(Messages):
    ...
    def __init__(self):
        ...
        self._metadata_dir = 'metadata'  # Директория для метаданных
            ...
            if out is True:
                ...
                bar = progressbar.ProgressBar(max_value = int(num_bars),
                                              ...
                                              widgets = [progressbar.SimpleProgress(
                                                  format =
                                                  '%(value_s)s ' + self._('из') + ' %(max_value_s)s (%(percentage)1d%%)'
                                              )]).start()
        ...
        if len(commands) > 0:
            ...
            if out is True:
                ...
                bar = progressbar.ProgressBar(
                    ...
                    initial_value = i,
                    widgets = [progressbar.SimpleProgress(
                        format = '%(value_s)s ' + self._('из') + ' %(max_value_s)s (%(percentage)1d%%)'
                    )]).start()
